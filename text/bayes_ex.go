...
func (b *NaiveBayesEx) getWords(words ...string) map[string]Word {
	return b.Words
}

func (b *NaiveBayesEx) setWord(word string, w Word, update bool) {
	b.Words[word] = w
}
...
func (b *NaiveBayesEx) Predict(sentence string) uint8 {
...
func (b *NaiveBayesEx) Probability(sentence string) (uint8, float64) {
	sums := make([]float64, b.classCount())
	for i := range sums {
		sums[i] = 1
	}
...
func (b *NaiveBayesEx) TopProbabilities(sentence string, topN int) []*Probability {
	sums := make([]float64, b.classCount())
	for i := range sums {
		sums[i] = 1
	}
...
// OnlineLearn lets the NaiveBayesEx model learn
// from the datastream, waiting for new data to
// come into the stream from a separate goroutine
func (b *NaiveBayesEx) OnlineLearn(errors chan<- error) {
	if errors == nil {
